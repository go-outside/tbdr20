/*
 * Tweaked to allow for disadvantage and also modification of concentration modifier at roll time
 * Version 0.1.16
...
var Concentration = Concentration || (function() {
  ...
  const kAdvantage = 'advantage';
  const kDisadvantage = 'disadvantage';
  ...
  addNetModifierInput = ( defaultModifier ) => {
    return '?{Net Concentration Modifier|' + defaultModifier + '}';
  },
  ...
  roll = (represents, DC, net_modifier, name, target, advantage) => {
      sendChat(script_name, '[[1d20cf<'+(DC-net_modifier-1)+'cs>'+(DC-net_modifier-1)+'+'+net_modifier+']]', results => {
          ...
          if( advantage !== undefined ){
              ...
              result = advantage === kAdvantage
                ? Math.max( rollResult, advantageRollResult )
                : Math.min( rollResult, advantageRollResult );
          }

          let total = result + net_modifier;
          ...
          let rollResultString = rollResult;
          if ( advantage !== undefined ) {
            rollResultString = maybeBold( Math.min( rollResult, advantageRollResult ), advantage == kDisadvantage )
              + ' / ' + maybeBold( Math.max( rollResult, advantageRollResult ), advantage == kAdvantage );
          }
          ...
          <div style="text-align: center"> \
              <b style="font-size: 16pt;"> \
                  <span style="border: 1px solid '+result_color+'; padding-bottom: 2px; padding-top: 4px;">[['+result+'+'+net_modifier+']]</span><br><br> \
                  '+result_text+' \
              </b> \
          </div>'
